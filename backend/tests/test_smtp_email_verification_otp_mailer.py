...
from app.integrations.smtp_email_verification_otp_mailer import (
...
def test_smtp_otp_mailer_sends_code_only_in_body(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "app.integrations.smtp_email_verification_otp_mailer.smtplib.SMTP",
        FakeSMTP,
    )
    mailer = SmtpEmailVerificationOtpMailer(
    ...
    mailer.send_email_verification_otp(
        ...
        verification_code=SecretStr(code),
    ...
    assert "Mealio" in smtp.message.get_content()
...
def test_smtp_otp_mailer_raises_generic_secret_free_error(
    monkeypatch: pytest.MonkeyPatch,
    ...
    with pytest.raises(EmailVerificationOtpDeliveryError) as exc_info:
        mailer.send_email_verification_otp(
            recipient_email="private-user@example.com",
            verification_code=SecretStr("654321"),
            expires_at=datetime.now(UTC) + timedelta(minutes=10),
        )

    public_error = str(exc_info.value)
    assert public_error == "Email verification code delivery failed"
    for secret in (
        "654321",
        "mailer-password",
...
def test_smtp_otp_mailer_rejects_naive_expiration() -> None:
    mailer = SmtpEmailVerificationOtpMailer(
        host="smtp.example.com",
        port=587,
        username=None,
