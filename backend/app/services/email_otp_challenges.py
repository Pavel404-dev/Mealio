...
class EmailOtpChallengeService:
    ...
    async def issue_challenge(
        ...
        *,
        ...
        async with self.db.begin():
            ...
            return await self.issue_challenge_in_transaction(
                user_id=user_id,
                purpose=purpose,
                target_email=target_email,
            )

    async def issue_challenge_in_transaction(
        self,
        *,
        user_id: uuid.UUID,
        purpose: EmailOtpPurpose,
        ...
        if not self.db.in_transaction():
            raise RuntimeError("Email OTP issuance requires an active transaction")
        ...
        if latest is not None and self._is_same_delivery_window(
            ...
            if latest.send_count >= self.settings.email_otp_max_deliveries_per_window:
    ...
    async def verify_and_consume(
        ...
        *,
        ...
        async with self.db.begin():
            return await self.verify_and_consume_in_transaction(
                user_id=user_id,
                purpose=purpose,
                target_email=target_email,
                code=code,
    ...
    async def verify_and_consume_in_transaction(
        self,
        *,
        user_id: uuid.UUID,
        purpose: EmailOtpPurpose,
        ...
        if not self.db.in_transaction():
            raise RuntimeError("Email OTP verification requires an active transaction")
    ...
    async def revoke_unused_for_target_in_transaction(
        self,
        *,
        user_id: uuid.UUID,
        purpose: EmailOtpPurpose,
